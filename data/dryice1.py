    ...
    def __getitem__(self, idx):
        ...
        if cam is not None:
            if "camera" in self.key_filter:
                # camera data
                result["camera_rotation"] = np.dot(self.transf[:3, :3].T, self.cam_rot[cam].T).T
                result["camera_position"] = np.dot(self.transf[:3, :3].T, self.campos[cam] - self.transf[:3, 3])
                result["focal"] = self.focal[cam]
                result["principal_point"] = self.princ_pt[cam]
                result["camera_index"] = self.all_cameras.index(cam)
