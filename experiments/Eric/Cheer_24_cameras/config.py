...
def get_dataset(camera_list=None, frame_list=None, background=False, use_depth=False, subsample_type=None, animation=ANIMATION):
    ...
    return Dataset(
        ...
        subsample_size=128,
        path=os.path.join('experiments', MODEL, 'data', animation))
...
class Train:
    ...
    def get_dataset(self):
        return get_dataset(camera_list=sorted([i for i in range(1, parameters.CAMERAS_NUMBER, 2)] +
                                              [j+1 for j in range(1, parameters.CAMERAS_NUMBER, 4)]),
...
class Progress:
    ...
    def get_dataset(self): return get_dataset(camera_list=sorted([i for i in range(1, parameters.CAMERAS_NUMBER, 2)] +
                                                                 [j+1 for j in range(1, parameters.CAMERAS_NUMBER, 4)]),
